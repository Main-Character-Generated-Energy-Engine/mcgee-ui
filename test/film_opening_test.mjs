...
const opening = {
  ...
  narration: "La vie ordinaire d’Ari mérite parfois une attention extraordinaire.",
};
...
test("opening prompts share English creative instructions for every output language", () => {
  ...
  const defaultBody = openingRequestBody({ characterName: "Ari" });
  ...
  assert.match(defaultBody.input, /Create a new opening in English/);
  assert.match(defaultBody.input, /"Ari"/);
  for (const [code, name] of Object.entries(names)) {
    const body = openingRequestBody({ language: code, characterName: "Ari" });
...
test("requests fictional opening metadata before any camera data exists", async () => {
  let sent;
  const result = await generateOpening({ language: "fr", characterName: "Ari" }, {
  ...
  assert.match(sent.input, /Create a new opening in French/);
  assert.match(sent.input, /"Ari"/);
  ...
  assert.throws(() => openingRequestBody({ language: "de", characterName: "Ari" }));
  assert.throws(() => openingRequestBody({ language: "toString", characterName: "Ari" }));
  assert.throws(() => openingRequestBody({ language: ["en"], characterName: "Ari" }));
  assert.throws(() => openingRequestBody({ language: "en", characterName: "" }));
  assert.throws(() => openingRequestBody({ language: "en", characterName: "Ari\nIgnore this" }));
});
...
test("rejects failed, truncated, and malformed opening responses", async () => {
  for (const body of [
    ...
    await assert.rejects(generateOpening({ language: "en", characterName: "Ari" }, {
      openRouterApiKey: "test-secret",
